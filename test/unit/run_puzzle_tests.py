...
def main():
  ...
  with tempfile.TemporaryDirectory(prefix="rypuztest_") as outdir:
    ...
    # (4) a solution that still contains FILL marks is rejected
    passed, out = chk(rypuzchk, symiri, base_puzzle, base_puzzle)
    check(
    ...
    if m:
      ...
      if n == 1:
        ...
        # Stage validation: FAIL_PARSE (syntax error inside a mask position)
        parse_path = os.path.join(outdir, "mut_parse_error.sir")
        err_syntax_gt = re.sub(
          r"(?<!\d)" + re.escape(val) + r"(?!\d)", "+ +", gt_text, count=1
        )
        with open(parse_path, "w") as f:
          f.write(err_syntax_gt)
        passed, out = chk(rypuzchk, symiri, base_puzzle, parse_path)
        check(
        ...
        # Stage validation: FAIL_COMPILE (typecheck error inside a mask position)
        compile_path = os.path.join(outdir, "mut_compile_error.sir")
        err_type_gt = re.sub(
          r"(?<!\d)" + re.escape(val) + r"(?!\d)", "3.14", gt_text, count=1
        )
    ...
    if m_fill:
      ...
      if cnt >= 1:
        ...
        puz_path = os.path.join(outdir, "mut_budget_puz.sir")
        with open(puz_path, "w") as f:
          f.write(mut_puz)
        passed, out = chk(rypuzchk, symiri, puz_path, base_gt)
        check(
    ...
    for i in range(1, len(parts), 2):
      ...
      if "entry" in header or "exit" in header:
        continue
      m_br = re.search(r"(\bbr\s+[^;]*\^)(\w+)", body)
      if m_br:
        mut_body = body[: m_br.start(2)] + "exit" + body[m_br.end(2) :]
        parts[i + 1] = mut_body
        mutated = True
        break
    if mutated:
      cfg_path = os.path.join(outdir, "mut_cfg.sir")
      with open(cfg_path, "w") as f:
        f.write("".join(parts))
      passed, out = chk(rypuzchk, symiri, base_puzzle, cfg_path)
      check(
    ...
    for i in range(1, len(parts), 2):
      ...
      if "entry" in header or "exit" in header:
        continue
      m_br = re.search(r"(br\s+[^,]+,\s*\^)(\w+)(,\s*\^)(\w+)(\s*;)", body)
      if m_br:
        swapped_body = (
          body[: m_br.start(2)]
          + m_br.group(4)
          + body[m_br.end(2) : m_br.start(4)]
          + m_br.group(2)
          + body[m_br.end(4) :]
        )
        ...
        break
    if mutated:
      path_path = os.path.join(outdir, "mut_path.sir")
      with open(path_path, "w") as f:
        f.write("".join(parts))
      passed, out = chk(rypuzchk, symiri, base_puzzle, path_path)
      check(
    ...
    if len(distinct_ints) >= 2:
      ...
      funcs = re.split(r"(fun\s+@\w+)", gt_text)
      if len(funcs) >= 3:
        swapped_body = re.sub(
        ...
        out_path = os.path.join(outdir, "mut_output.sir")
