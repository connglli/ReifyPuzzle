...
- **Memory intrinsics** (`@memcpy`, `@memset`). Currently classified P4 in [`docs/intrinsics.md`](./intrinsics.md): their SMT encoding requires byte-level array reasoning with potentially symbolic sizes. Eligible for re-promotion when the solver gains byte-addressable memory.
