...
- Batch D — §12 *floating-point basic IEEE family*: sign / bit ops
  (D.1), classification predicates (D.2), min / max (D.3), and
  correctly-rounded math (D.4) shipped (§12.6): `@fabs`, `@fneg`,
  `@copysign`, `@signbit`, `@to_bits`, `@from_bits`, `@is_normal`,
  `@is_subnormal`, `@fmin`, `@fmax`, `@sqrt`, `@floor`, `@ceil`,
  `@trunc`.  Shipping D.1 opens the type-restriction sentence at the
  top of §12 to `fN`; the remaining members (D.5 compositions:
  `@fract`, `@recip`) are planned as a follow-up sub-batch.  The
  exponent-manipulation candidates (`@ldexp`, `@scalbn`, `@ilogb`,
  `@logb`) were **dropped** — symbolic `2^exp` scaling and exponent
  extraction have no clean QF_FP encoding, so they are not
...
- **Additional intrinsics — P0 tier, planned for v0.2.2 follow-up batches.** Integer extras (`@abs_diff`, `@signum`, `@umin`, `@umax`, `@clamp`, `@midpoint`), bit-manipulation (`@parity`, `@bswap`, `@bitreverse`, `@rotl`, `@rotr`, `@is_pow2`, `@ilog2`), the integer overflow-aware family (`@wrapping_*`, `@checked_*`, `@saturating_*`, `@overflowing_*`, `@widening_mul`, `@div_euclid`, `@rem_euclid`), and the floating-point basic IEEE family (`@fabs`, `@copysign`, `@fmin`, `@fmax`, `@sqrt`, `@floor`/`@ceil`/`@trunc`, `@signbit`, `@is_normal`, `@is_subnormal`, `@to_bits`/`@from_bits`, `@fract`, `@recip`) are all solver-easy and broadly target-easy. They are slated for follow-up batches in the v0.2.2 line. The D.1 sub-batch (sign / bit ops: `@fabs`, `@fneg`, `@copysign`, `@signbit`, `@to_bits`, `@from_bits`), D.2 (classification predicates: `@is_normal`, `@is_subnormal`), D.3 (min / max: `@fmin`, `@fmax`), and D.4 (correctly-rounded math: `@sqrt`, `@floor`, `@ceil`, `@trunc`) have shipped; D.1 opens the §12 type-restriction sentence to `fN`. The remaining D sub-batch (D.5 compositions: `@fract`, `@recip`) is planned. The exponent-manipulation candidates (`@ldexp`, `@scalbn`, `@ilogb`, `@logb`) were dropped — symbolic `2^exp` scaling and exponent extraction have no clean QF_FP encoding, so they are not solver-friendly intrinsic targets. See [`docs/intrinsics.md`](./intrinsics.md) for the per-batch scope.
