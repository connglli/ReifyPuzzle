...
**Target soundness.** All three backends preserve the divergence, so a diverging program that *terminates* once compiled is a miscompilation. WASM and Python have no forward-progress assumption. The C backend is sound too — a subtle point worth stating: C11 §6.8.5p6 lets a compiler assume a loop terminates (and so delete a side-effect-free one) *only when its controlling expression is not a constant*. RefractIR never emits such a loop for the divergence: the default lowering is `label: … goto label`, and `--structured-lowering` emits `for (;;)` / `while (1)` + `break` — both have a constant (or absent) controlling expression, so the exemption does not apply and gcc/clang must keep the loop. Verified across gcc/clang `-O0`/`-O2` (goto and structured) and `wasmtime`. (This relies on the output being compiled as **C**, not C++, whose `[intro.progress]` has no constant-expression carve-out.) An `@observe` intrinsic (`docs/intrinsics.md` §12.7) is available to plant an unremovable observable `volatile` write in the cycle as belt-and-suspenders; the constant-condition lowering makes it unnecessary for the divergence to survive, so rysmith does not emit it by default.
