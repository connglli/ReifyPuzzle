...
namespace symir {
  ...
  FunDecl Parser::parseFunDecl() {
    ...
    return FunDecl{std::move(name),          std::move(params), std::move(ret),
                   std::move(syms),          std::move(lets),   std::move(blocks),
                   SourceSpan{b, prevEnd()}, std::string{},     FunDecl::Attributes{}};
  }
